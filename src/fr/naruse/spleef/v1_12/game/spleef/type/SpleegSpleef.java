...
public class SpleegSpleef extends Spleef implements Listener {
    public SpleegSpleef(SpleefPluginV1_12 pl, String name, Location spleefLoc, Location spleefSpawn, Location spleefLobby, int min, int max, boolean isOpen) {
    ...
    public void blockChange(ProjectileLaunchEvent e){
        ...
        if(getAuthorizedMaterial().contains(hitBlock.getType()) && getAuthorizedData().containsKey(hitBlock.getType()) && getGame().GAME){
            if(getAuthorizedData().get(hitBlock.getType()) == hitBlock.getData()) {
                return;
            }
        }
        ...
        getDataOfLocationHashMap().put(hitBlock.getLocation(), hitBlock.getData());
    }
