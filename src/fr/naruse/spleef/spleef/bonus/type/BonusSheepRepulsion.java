...
public class BonusSheepRepulsion extends BonusColored {
    ...
    protected void onTick() {
        ...
        if(sheep != null && !sheep.isDead()){
            sheep.setTarget(p);
            List<Entity> stream = getNearbySheeps(sheep.getLocation(), 10, 5, 10).filter(entity -> entity != sheep).collect(Collectors.toList());;
            runSync(() -> {
